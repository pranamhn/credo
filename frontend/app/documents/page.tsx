            ...
            <div className="flex items-center justify-between gap-3 border-b border-slate-100 bg-slate-50/50 px-4 py-2.5">
              <div className="relative">
                ...
                <input
                  ...
                  className="rounded-lg border border-slate-200 bg-white py-1.5 pl-8 pr-3 text-xs text-slate-700 outline-none focus:border-violet-400 w-52"
                />
              </div>
    ...
    <td className="px-4 py-3.5">
      <div className="relative">
        <button onClick={() => setOpen((v) => !v)}
          className={cn("flex items-center gap-1.5 rounded-lg px-2 py-1 text-[11px] font-medium transition-colors border",
            ...
            <ActionMenu id={s.id}
              extra={s.status !== "queued" && s.status !== "parsing" && (
                <button onClick={() => handleReparse(s.id)}
                  className="inline-flex items-center gap-1 rounded-lg border border-slate-200 bg-white px-2.5 py-1.5 text-[11px] font-semibold text-amber-700 hover:bg-amber-50 transition-colors">
      ...
      <tbody>
        {rows.map((s) => (
          <tr key={s.id} className={cn("border-b border-slate-100 hover:bg-slate-50 transition-colors", selected.has(s.id) && "bg-violet-50/40")}>
            ...
            <td className="px-4 py-3.5">
              <span className="inline-flex rounded-full bg-slate-100 px-2 py-0.5 text-[11px] font-medium text-slate-500">{s.document_type ?? "other"}</span>
            </td>
