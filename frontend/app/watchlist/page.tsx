...
import { localData, WatchlistEntry } from "@/lib/localData";
import { Plus, Trash2, Search, AlertTriangle, Target, Calendar } from "lucide-react";
...
export default function WatchlistPage() {
  ...
  useEffect(() => { setEntries(localData.getWatchlist()); }, []);
        ...
        <div className="flex items-center gap-3 flex-wrap">
          ...
          {(["all","kuning","merah"] as const).map((t) => (
            <button key={t} onClick={() => setTierFilter(t)}
              className={`px-3 py-2 rounded-lg text-xs font-semibold transition-all ${tierFilter === t ? "bg-teal-50 text-teal-700 ring-1 ring-teal-200" : "text-slate-500 hover:bg-slate-100 border border-slate-200"}`}>
        ...
        {editId !== null && (
          <div className="rounded-xl border border-teal-200 bg-teal-50/50 p-6 shadow-sm">
            ...
            <div className="grid sm:grid-cols-2 gap-4">
              {[
                { label: "Nama Perusahaan *", node: <input value={draft.companyName} onChange={(e) => setDraftField("companyName", e.target.value)} placeholder="PT ..." className="w-full text-xs border border-slate-200 rounded-lg px-3 py-2 text-slate-700 outline-none focus:border-teal-400 bg-white" /> },
                { label: "Company ID (opsional)", node: <input value={draft.companyId} onChange={(e) => setDraftField("companyId", e.target.value)} placeholder="UUID dari Companies" className="w-full text-xs border border-slate-200 rounded-lg px-3 py-2 text-slate-700 outline-none focus:border-teal-400 bg-white" /> },
                { label: "Tier EWS", node: (
                ...
                { label: "PIC / Assignee", node: <input value={draft.assignee} onChange={(e) => setDraftField("assignee", e.target.value)} placeholder="Nama analis" className="w-full text-xs border border-slate-200 rounded-lg px-3 py-2 text-slate-700 outline-none focus:border-teal-400 bg-white" /> },
                { label: "Tanggal Ditambahkan", node: <input type="date" value={draft.addedDate} onChange={(e) => setDraftField("addedDate", e.target.value)} className="w-full text-xs border border-slate-200 rounded-lg px-3 py-2 text-slate-700 outline-none focus:border-teal-400 bg-white" /> },
                { label: "Target Penyelesaian", node: <input type="date" value={draft.targetDate} onChange={(e) => setDraftField("targetDate", e.target.value)} className="w-full text-xs border border-slate-200 rounded-lg px-3 py-2 text-slate-700 outline-none focus:border-teal-400 bg-white" /> },
              ].map(({ label, node }) => (
                <div key={label}>
              ...
              <div className="sm:col-span-2">
                ...
                <div className="flex gap-2 mb-2">
                  <input value={newReason} onChange={(e) => setNewReason(e.target.value)}
                    ...
                    className="flex-1 text-xs border border-slate-200 rounded-lg px-3 py-2 text-slate-700 outline-none focus:border-teal-400 bg-white" />
              ...
              <div className="sm:col-span-2">
                ...
                <textarea value={draft.actionPlan} onChange={(e) => setDraftField("actionPlan", e.target.value)} rows={2}
                  ...
                  className="w-full text-xs border border-slate-200 rounded-lg px-3 py-2 text-slate-700 outline-none focus:border-teal-400 bg-white resize-none placeholder:text-slate-300" />
              </div>
              <div className="sm:col-span-2">
            ...
            <button onClick={saveEntry}
              className="mt-4 flex items-center gap-2 bg-teal-500 text-white text-xs font-semibold px-5 py-2.5 rounded-lg hover:bg-teal-600 transition-all">
                ...
                <div key={e.id} className={`rounded-xl border bg-white p-5 shadow-sm ${e.ewsTier === "merah" ? "border-red-200" : "border-amber-200"}`}>
                  <div className="flex items-start justify-between gap-4">
                    <div className="flex items-start gap-3 flex-1">
                      ...
                      <div className="flex-1">
                        <div className="flex items-center gap-2">
                          ...
                          {e.companyId && (
                            <Link href={`/companies/${e.companyId}`} className="text-[10px] text-teal-600 hover:underline">→ Profil</Link>
                          )}
                        </div>
                        {e.reasons.length > 0 && (
                    ...
                    <div className="flex items-center gap-2 shrink-0">
                      ...
                      <button onClick={() => startEdit(e)} className="text-xs text-teal-600 hover:underline font-medium">Edit</button>
