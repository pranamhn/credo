...
import { Printer, PencilLine, ShieldCheck, Plus, X } from "lucide-react";
import { formatIDR, formatDate } from "@/lib/utils";
import type { CompanySummary, SlikReport, CbiReport } from "@/lib/api";
import type { CompanyProfile, CreditMemo, ScoringAspect, DebtEntry } from "@/lib/localData";
...
export function CreditSummaryTab({
  ...
  updateDscrCicilanBaru, addDebtEntry, updateDebtEntry, removeDebtEntry,
  updateScoringAspect,
}: Props) {
  return (
    <div className="p-6 space-y-5">
      <div className="flex justify-end">
        <button
          ...
          className="inline-flex h-9 items-center gap-2 rounded-lg border border-slate-200 bg-white px-3 text-xs font-semibold text-slate-600 transition-colors hover:border-teal-300 hover:text-teal-700"
        >
      ...
      {profile.ringkasanEksekutif && (
        <div className="rounded-xl border border-teal-200 bg-teal-50/40 p-4">
          <p className="text-[10px] font-semibold uppercase tracking-widest text-teal-600 mb-2">Ringkasan Eksekutif</p>
      ...
      <div className="rounded-xl border border-slate-200 bg-white p-5">
        <div className="flex items-center justify-between gap-4">
          ...
          <div className="flex flex-col items-end gap-1.5 shrink-0">
            <div className={`inline-flex items-center gap-1.5 rounded-full px-2.5 py-1 text-[10px] font-bold ring-1 ${
              ewsTier === "hijau" ? "bg-emerald-50 text-emerald-700 ring-emerald-200"
      ...
      <div className="rounded-xl border border-slate-200 bg-white overflow-hidden">
        ...
        <div className="p-4 space-y-3">
          {([
            ...
            const barColor = pct >= 75 ? "bg-emerald-500" : pct >= 50 ? "bg-teal-500" : pct >= 25 ? "bg-amber-500" : "bg-red-500";
            return (
              <div key={label}>
        ...
        <div className="rounded-xl border border-slate-200 bg-white overflow-hidden">
          <div className="px-5 py-3 border-b border-slate-100 bg-slate-50/60 flex items-center justify-between">
            ...
            <span className={`text-[10px] px-2 py-0.5 rounded-full font-bold ring-1 ${
              slikDerived.worstKol === 1 ? "bg-emerald-50 text-emerald-700 ring-emerald-200"
                      ...
                      return (
                        <tr key={r.id} className="border-b border-slate-50 last:border-b-0">
                          <td className="px-3 py-2">
                            <span className="inline-flex px-1.5 py-0.5 rounded text-[10px] font-bold bg-blue-50 text-blue-700">SLIK</span>
                          </td>
                          ...
                          <td className="px-3 py-2 text-center">
                            <span className={`inline-flex px-1.5 py-0.5 rounded text-[10px] font-bold ${
                              wk === 1 ? "bg-emerald-50 text-emerald-700"
              ...
              <tbody>
                {pnlComparison.rows.map((row) => (
                  <tr key={row.key} className={`border-b border-slate-50 last:border-b-0 ${row.isBold ? "bg-slate-50/60" : ""}`}>
                    ...
                    <td className={`px-4 py-2.5 text-right tabular-nums whitespace-nowrap ${row.isBold ? "font-bold text-slate-900" : "text-slate-700"} ${Number(row.latestTotal ?? 0) < 0 ? "text-red-600" : ""}`}>
      ...
      <div className="rounded-xl border border-slate-200 bg-white overflow-hidden">
        ...
        {(() => {
          const ebitdaRaw = pnlComparison?.rows.find((r) => r.key === "operating_profit")?.latestTotal ?? null;
          const latestMo  = pnlComparison?.latestMo ?? 12;
          const ebitda    = ebitdaRaw !== null && latestMo < 12 ? (ebitdaRaw / latestMo) * 12 : ebitdaRaw;
          const existingMonthly = debtEntries.reduce((s, e) => s + e.cicilanPerBulan, 0);
          ...
          const dscrColor       = dscr === null ? "text-slate-400"
            : dscr >= 1.5 ? "text-emerald-600" : dscr >= 1.2 ? "text-teal-600"
          ...
          const dscrBadge = dscr === null ? null
            ...
            : dscr >= 1.2 ? <span className="inline-flex rounded-full px-2 py-0.5 text-[10px] font-bold bg-teal-50 text-teal-700 ring-1 ring-teal-200">KUAT</span>
              ...
              <table className="w-full text-xs">
                ...
                <tbody>
                  <tr className="border-b border-slate-50 hover:bg-slate-50/40">
                    <td className="px-4 py-2.5 text-slate-700">Operating Cash Flow / EBITDA</td>
                  ...
                  <tr className="border-b border-slate-50 hover:bg-slate-50/40">
                    ...
                    <td className="px-4 py-2.5 text-right">
                      <input
                        ...
                        placeholder="0"
                        className="w-full rounded-lg border border-slate-200 px-2.5 py-1 text-[11px] text-right text-slate-700 outline-none focus:border-teal-400 tabular-nums placeholder:text-slate-300"
                      />
                    </td>
        ...
        <div className="rounded-xl border border-slate-200 bg-white overflow-hidden">
          <div className="px-5 py-3 border-b border-slate-100 bg-slate-50/60 flex items-center justify-between">
            ...
            {(() => {
              ...
              const colorClass = weighted >= 4 ? "text-emerald-600" : weighted >= 3 ? "text-teal-600" : weighted >= 2 ? "text-amber-600" : "text-red-600";
                    ...
                    <tr key={aspect.id} className="border-b border-slate-50 last:border-b-0">
                        ...
                        <div className="flex items-center justify-center gap-0.5">
                          {[1, 2, 3, 4, 5].map((s) => (
                            <button key={s} type="button" onClick={() => updateScoringAspect(idx, "skor", s)}
                              className={`h-6 w-6 rounded text-[10px] font-bold transition-all ${aspect.skor === s ? "bg-teal-50 text-teal-700 ring-1 ring-teal-200" : "bg-slate-100 text-slate-400 hover:text-slate-600"}`}>
                              {s}
                            </button>
                          ))}
                        </div>
                      ...
                      <td className="px-4 py-2.5">
                        <input value={aspect.catatan}
                          ...
                          className="w-full rounded border border-slate-200 px-2 py-1 text-[11px] text-slate-700 outline-none focus:border-teal-400 placeholder:text-slate-300" />
                      </td>
                    </tr>
                  );
                })}
                  ...
                  <tr key={entry.id} className="border-b border-slate-50 last:border-b-0">
                    <td className="px-3 py-2">
                      <input value={entry.kreditur} onChange={(e) => updateDebtEntry(entry.id, "kreditur", e.target.value)}
                        placeholder="Nama bank..." className="w-full min-w-[100px] rounded border border-slate-200 px-2 py-1 text-[11px] text-slate-700 outline-none focus:border-teal-400 placeholder:text-slate-300" />
                    </td>
                    <td className="px-3 py-2">
                      <input value={entry.fasilitas} onChange={(e) => updateDebtEntry(entry.id, "fasilitas", e.target.value)}
                        placeholder="KMK, KI..." className="w-full min-w-[80px] rounded border border-slate-200 px-2 py-1 text-[11px] text-slate-700 outline-none focus:border-teal-400 placeholder:text-slate-300" />
                    </td>
                    {(["plafon", "outstanding", "cicilanPerBulan"] as const).map((field) => (
                      <td key={field} className="px-3 py-2">
                        <input type="number" value={entry[field] || ""} onChange={(e) => updateDebtEntry(entry.id, field, Number(e.target.value))}
                          placeholder="0"
                          className="w-full min-w-[110px] rounded border border-slate-200 px-2 py-1 text-[11px] text-right text-slate-700 outline-none focus:border-teal-400 tabular-nums placeholder:text-slate-300" />
                      </td>
                    ))}
                    <td className="px-3 py-2">
          ...
          <div className="grid gap-3 sm:grid-cols-4">
            <div>
              ...
              <input type="number" value={dscrCicilanBaru || ""}
                onChange={(e) => updateDscrCicilanBaru(Number(e.target.value))}
                placeholder="0"
                className="w-full rounded-lg border border-slate-200 px-3 py-2 text-xs text-slate-700 outline-none focus:border-teal-400 tabular-nums placeholder:text-slate-300" />
            </div>
            {(() => {
              ...
              const dscrColor = dscr === null ? "text-slate-400" : dscr >= 1.5 ? "text-emerald-600" : dscr >= 1.2 ? "text-teal-600" : dscr >= 1.0 ? "text-amber-600" : "text-red-600";
              return (
                <>
                  <div>
      ...
      {(profile.riskAnalystName || profile.cooName || profile.ceoName) && (
        <div className="rounded-xl border border-slate-200 bg-white overflow-hidden">
          <div className="px-5 py-3 border-b border-slate-100 bg-slate-50/60">
          ...
          <div className="p-5 grid grid-cols-1 sm:grid-cols-3 divide-y sm:divide-y-0 sm:divide-x divide-slate-100">
            {[
              { jabatan: "Risk Analyst", nama: profile.riskAnalystName, tanggal: profile.riskAnalystDate },
              { jabatan: "COO",          nama: profile.cooName,          tanggal: profile.cooDate          },
              { jabatan: "CEO",          nama: profile.ceoName,          tanggal: profile.ceoDate          },
            ].map(({ jabatan, nama, tanggal }) => (
              <div key={jabatan} className="flex flex-col items-center gap-3 py-4 sm:py-0 px-5 first:pl-0 last:pr-0">
                ...
                <div className="w-full border-t-2 border-slate-300 pt-2 text-center">
                  ...
                  {tanggal && <p className="text-[10px] text-slate-400 mt-0.5">{formatDate(tanggal)}</p>}
                </div>
              </div>
            ))}
          </div>
